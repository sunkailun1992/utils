...
9. `docs/ai-coding/VERSIONING_SPEC.md`：确认 `group = 'com.kellen'`、补丁递增和消费者同步规则。
...
- 业务微服务表设计规范由 `docs/ai-coding/PROJECT_CODING_SPEC.md` 和 `docs/ai-coding/examples` 提供模板；新增表必须包含公共治理字段，`version` 只做乐观锁，业务版本使用 `*_version` 命名。
...
- 从 `main` 分支开始执行提交并推送 Git 仓库时，必须同时执行远程私有 Maven 仓库发布，保证 Git 代码和远程 `com.kellen:utils:<version>` 制品一致。
